...
## 260220-04:52:29 - Surface socket glob errors and permission-denied sockets

### Summary
Socket discovery now reports malformed glob config and preserves permission-denied sockets as visible partial failures.

### Added
- Added regression tests for malformed `-socket-glob` handling and tmux-style permission-denied socket errors.

### Changed
- Changed socket discovery to return discovery errors alongside discovered socket targets.
- Changed session listing to include discovery errors in fatal/partial socket error output.

### Fixed
- Fixed unavailable-socket classification to avoid swallowing permission-denied errors as unavailable sockets.
- Fixed malformed `-socket-glob` patterns being ignored silently.
...
### QA Notes
- Verify malformed `-socket-glob` now surfaces an error in status output.
- Verify a permission-denied socket still shows healthy sessions and reports `partial socket failures`.

## 260220-05:05:33 - Include active TMUX socket fallback for multi-socket discovery

### Summary
When default socket is unavailable in a non-default tmux client, discovery now also probes the current TMUX socket so sessions still load.

### Added
- Added discovery fallback to include the current TMUX socket path when `-include-default-socket` is enabled.
- Added regression tests for TMUX-env socket discovery and fallback session listing.

### Changed
- Made socket tests deterministic by clearing inherited `TMUX` env in tests that assume no active client socket.
...
### QA Notes
- Verify running visualiser inside `tmux -L <custom>` still shows sessions from that active socket even if default has no server.
- Verify default socket remains included and deduped when active TMUX socket is canonical default.
- Verify socket discovery tests pass inside and outside tmux.
